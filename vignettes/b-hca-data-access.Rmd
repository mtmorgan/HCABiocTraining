...
The [Seurat][] package represents data as a `Seurat` object. Normally,
a `Seurat` object can be created using [SeuratDisk][], but this
currently does not handle the `.h5ad` file format made available by
CELLxGENE. The helper function `training_read_h5ad_as_seurat()`
sketches an approach to data import using the [anndata][] *R* package
and [anndata][anndata-python] module.
